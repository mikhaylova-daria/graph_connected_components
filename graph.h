...
struct Edge {
    ...
    U weight;
    Edge(){;}
    Edge (int _first_v, int _second_v, U _weight = U(0)):id(((_first_v + _second_v) * (_first_v + _second_v) + 3 * _first_v + _second_v)/2 ),
...
struct Vetex {
    ...
    int color = 0 ;
    Vetex():color(0){;}
    Vetex(int _id, T _value =  T(0)): id(_id), value(_value), color(0){;}
...
public:
    ...
    std::list<std::set<Vetex<T> > >coherency();
    Vetex<T> * find(int id) {
        typename map <int, Vetex<T> >::iterator itr;
        if ((itr = map_vtx.find(id)) != map_vtx.end()) {
            return &(*itr).second;
        } else {
            throw ("not found");
            return &(*map_vtx.begin()).second;
        }
    }
...
template < typename T, typename U>
   std::list<std::set<Vetex<T> > > Graph<T, U>::coherency() {
       std::list<std::set<Vetex<T> > > answer;
       Graph <T, U> B(*this);
       B.transpose();
       list <Vetex<T> > vetices;
       vetices = B.get_vetices();
       typename Graph <T, U>::iterator_DFS itr(this);
       typename Graph <T, U>::iterator_DFS itr_buf(&B);
       for (; itr != itr.end(); ++itr){
           set<Vetex<T> > sub_tree;
           Vetex <T>*  x = B.find((*itr).id);
           if (x->color == 0) {
               itr_buf.DFS_visit(x, true, &sub_tree);
               answer.push_back(sub_tree);
           }
           typename set<Vetex<T> >::iterator iterat;
           for (iterat = sub_tree.begin(); iterat != sub_tree.end(); ++iterat){
               B.find((*iterat).id)->color = 2;
           }
       }
       return answer;
   }
   ...
   template < typename T, typename U >
                  ...
                  iterator_BFS(Graph<T, U> * _gr,int id):gr(_gr) {
                       ...
                       while (!gray.empty()) {
                           ...
                           for (itr_i = list_as_start_of_current_gray_vtx.begin(); itr_i != list_as_start_of_current_gray_vtx.end(); ++itr_i){
                               ...
                               vector_iterator = std::find(tree_BFS.begin(), tree_BFS.end(), p_vtx);
          ...
          template < typename T, typename U >
                 class Graph<T, U>::iterator_DFS {
                     ...
                     void DFS_visit(Vetex<T>* x, bool flag = false,  set<Vetex<T> > * coh = NULL){
                     ...
                     friend class Graph<T, U>;
                         iterator_DFS(Graph<T, U> * _gr):gr(_gr) {
                             ...
                             for (itr_i = gr->map_vtx.begin(); itr_i != gr->map_vtx.end(); ++itr_i){
                                  (itr_i->second).color = 0;
                             }
                             for (itr_i = gr->map_vtx.begin(); itr_i != gr->map_vtx.end(); ++itr_i){
                                  Vetex<T> * p_vtx = & (itr_i->second);
                                  if (p_vtx->color == 0) { // если её ещё нет в дереве, значит вершина белая
                                      DFS_visit(p_vtx);
                                  }
                              }
                              ...
                              std::reverse(tree_DFS_black.begin(), tree_DFS_black.end());
                         }
